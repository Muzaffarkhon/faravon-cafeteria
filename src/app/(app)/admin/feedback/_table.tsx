...
import { Badge, Button, Card, Textarea, cx, type BadgeTone } from "@/components/ui";
import { FEEDBACK_STATUS_LABEL, FEEDBACK_STATUS_ORDER, FEEDBACK_STATUS_TONE } from "@/lib/feedback";
      ...
      {rows.map((f) => (
        <Card key={f.id} className="p-4">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {FEEDBACK_STATUS_ORDER.filter((s) => s !== "NEW" || f.status === "NEW").map((st) => (
              <Button
                key={st}
                size="sm"
                variant={f.status === st ? "primary" : "secondary"}
                disabled={pending || f.status === st}
                loading={busyId === f.id}
                onClick={() => apply(f.id, st)}
                className={cx(f.status === st && "pointer-events-none")}
              >
