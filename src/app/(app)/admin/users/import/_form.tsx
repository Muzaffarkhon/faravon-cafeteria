    ...
    <form action={formAction} className="max-w-xl space-y-4">
      ...
      <label className="flex items-start gap-2 text-sm text-ink">
        ...
        <span>
          ...
          <span className="block text-xs text-ink-muted">
            Соответствует §8: отсутствующий в источнике сотрудник теряет доступ. Их учётные записи
      ...
      {state.ok && (
        <div className="space-y-2 rounded-md border border-line bg-surface px-4 py-3 text-sm">
          <p className="font-medium text-success-strong">Импорт завершён.</p>
          <ul className="text-ink-muted">
            <li>Добавлено: {state.created ?? 0}</li>
            <li>Обновлено: {state.updated ?? 0}</li>
            {(state.deactivated ?? 0) > 0 && <li>Деактивировано: {state.deactivated}</li>}
          </ul>
