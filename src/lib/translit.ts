...
/**
 * Огрубляет ФИО до «звучания» — для нечёткого поиска, где буквы,
 * различимые только на письме, пишущий часто путает: специфичные
 * таджикские буквы с похожими на них обычными кириллическими
 * (ӣ/и, ӯ/у, ҳ/х, ҷ/ч, ғ/г, қ/к), а гласные а/о путают между собой
 * особенно часто (напр. «Арипов»/«Орипов» — одна и та же фамилия).
 * Сравнивать нужно и запрос, и кандидата этой же функцией.
 */
export function fuzzyNameKey(input: string): string {
  return input
    .toLowerCase()
    .replace(/ё/g, "е")
    .replace(/ӣ/g, "и")
