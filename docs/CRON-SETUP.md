...
|---|---|---|
| `/api/cron/deliver-notifications` | каждые 10 мин | добор недоставленных Telegram-уведомлений + оконные уведомления периода (§5.10) + автосмена периодов (§2): закрывает истёкший, открывает следующий по датам — ручное управление в admin/periods остаётся доступным |
